...
<div class="container">
    ...
    <a href="{{url('/titres/create')}}" class="btn btn-primary">create</a>
     @if(count($titres) != 0 ) 
    <table class="table table-striped" >
          ...
          @foreach($titres as $titre)
              <tr>
                <td>{{$titre->title}}</td>
              <td>{{$titre->categorie->title_categorie}}</td>
              <td>
                <form action="{{url('/titres/'.$titre->id)}}" method="post">
                    {{ csrf_field() }}
            
                    {{ method_field('DELETE') }}
              <button class="btn btn-danger" onclick="return confirm('');"><i class="fa fa-trash"></i></button>
              <a href="{{url('/titres/'.$titre->id.'/edite')}}" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
            </form>
              </td>
              </tr>
          @endforeach 
