...
<div class="container">
    ...
    <a href="{{url('/pubs/create')}}" class="btn btn-primary">create</a>
     @if(count($articles) != 0 ) 
    <table class="table table-striped" >
          ...
          @foreach($articles as $article)
              ...
              <td>
                <form action="{{url('/pubs/'.$article->id)}}" method="post">
                    {{ csrf_field() }}
            
                    {{ method_field('DELETE') }}
              <button class="btn btn-danger" onclick="return confirm('');"><i class="fa fa-trash"></i></button>
              <a href="{{url('/pubs/'.$article->id.'/edite')}}" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
            </form>
              </td>
              </tr>
          @endforeach 
